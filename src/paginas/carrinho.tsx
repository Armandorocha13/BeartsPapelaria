...
const Carrinho = () => {
    ...
    const handleCheckout = () => {
        ...
        const itemsList = items.map(item =>
            `• ${item.quantity}x ${item.name}${item.paperType && item.category !== 'dia-dos-namorados' ? ` (${item.paperType})` : ''}${item.handleType ? ` [${item.handleType}]` : ''} (${formatPrice(item.price * item.quantity)})`
        ).join('\n');
